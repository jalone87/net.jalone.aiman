...
public abstract class Pac extends Agent implements Steppable{
	
    /** How long we wait while the Pac dies (not spinning). */
    public static final int WAIT_TIME = 10; //was 100
        
    /** How long we wait while the Pac spins around while dying. */
    public static final int SPIN_TIME = 0; //was 100
    ...
    /** 
     * To give Pacs behaviors derived classes should implement this method
     */
    protected abstract void doPolicyStep(SimState state);
    ...
    public void step(SimState state)
        ...
        for(int i=0; i < nearby.numObjs; i++)
            ...
            if (obj instanceof Energizer && pacman.dots.getObjectLocation(obj).equals(location))  // uh oh
                ...
                pacman.dots.remove((Energizer)obj);
//                pacman.energizers.removeObjectsAtLocation(pacman.dots.getObjectLocation(obj));
            ...
            //EDITED AiMan TODO check if correct
            else if (obj instanceof Dot && pacman.dots.getObjectLocation(obj).equals(location))
                {
                pacman.score+=10;
    ...
    */
    ...
    public void die(){
        ...
        Logger.getInstance().log("-Game over. Score: " + pacman.score);
        ...
        pacman.schedule.scheduleOnceIn(WAIT_TIME * 2 + SPIN_TIME,
            new Steppable()
                {
                public void step(SimState state) { pacman.resetGame(); }
                });
        }
    }
